...
class SimpleGCXTool:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("GCX Tool - Original Match")
    ...
    def setup_gui(self):
        ...
        # File selection
        file_frame = ttk.LabelFrame(main_frame, text="Select GCX File")
        ...
        ttk.Button(file_inner, text="Browse", command=self.browse_file).pack(side='right', padx=(10,0))
        
        # Buttons
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill='x', pady=10)
        
        ttk.Button(button_frame, text="Extract Text", command=self.extract_text).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Export to Excel", command=self.export_excel).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Import Translations", command=self.import_translations).pack(side='left', padx=5)
    ...
    def extract_text(self):
        if not self.current_file:
            messagebox.showerror("Error", "Select GCX File أولاً")
            return
    ...
    def update_results(self):
        ...
        if self.current_gcx:
            ...
            self.status_var.set(f"Extracted {count} texts")
    
    def export_excel(self):
        if not self.current_gcx or not self.current_gcx.string_tables:
            messagebox.showerror("Error", "لا توجد textsوص مستخرجة")
            return
    ...
    def export_to_excel_file(self, filename):
        try:
            ...
            messagebox.showerror("Error", f"Failed التصدير: {e}")
    ...
    def import_translations(self):
        ...
        gcx_file = filedialog.askopenfilename(
            title="Select GCX File المستهدف",
        ...
        def worker():
            try:
                self.status_var.set("Importing translations...")
                ...
                if not translations:
                    self.root.after(0, lambda: messagebox.showwarning("Warning", "No translations found"))
                    return
                ...
                self.root.after(0, lambda: messagebox.showinfo("Success", f"Applied {applied} translations"))
                self.root.after(0, lambda: self.status_var.set(f"Applied {applied} translations"))
                
            except Exception as e:
                self.root.after(0, lambda: messagebox.showerror("Error", f"Failed الاستيراد: {e}"))
                self.root.after(0, lambda: self.status_var.set("Failed"))
